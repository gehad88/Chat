...
namespace server
{
    public partial class Form1 : Form
    {
        ...
        private async Task HandleClient(Socket client, int clientId)
        {
            ...
            try
            {
                ...
                while (true)
                {
                    ...
                    else if (request.StartsWith("Serialize"))
                    {
                        string filePath = request.Substring(9).Trim();
                        if (File.Exists(filePath))
                        {
                            SerializeFile(filePath);
                            data = Encoding.ASCII.GetBytes($"File {filePath} serialized.\n");
                        }
                        else
                        {
                            data = Encoding.ASCII.GetBytes("ERROR: File not found\n");
                    ...
                    else if (request.StartsWith("Deserialize"))
                    {
                        string filePath = request.Substring(11).Trim();
                        string deserializedFilePath = DeserializeFile(filePath);
                        if (!string.IsNullOrEmpty(deserializedFilePath))
                        {
                            await SendFile(deserializedFilePath, ns);
                        }
                        else
                        {
                            data = Encoding.ASCII.GetBytes("ERROR: Deserialization failed\n");
                            await ns.WriteAsync(data, 0, data.Length);
                        }
                    }
                }
        ...
        {
            string serializedFilePath = filePath + ".bin";
        ...
        private string DeserializeFile(string filePath)
        {
            string deserializedFilePath = filePath.Replace(".bin", ".txt");
            if (File.Exists(filePath))
            {
                using (FileStream fs = new FileStream(filePath, FileMode.Open))
                {
                    BinaryFormatter formatter = new BinaryFormatter();
                    string content = (string)formatter.Deserialize(fs);
                    File.WriteAllText(deserializedFilePath, content);
                }
