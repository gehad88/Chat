            ...
            try
            {
                ...
                while (true)
                {
                    ...
                    if (response.StartsWith("IMG:"))
                    {
                        ...
                        // Save image file to client directory
                        Directory.CreateDirectory("ClientDownloads");
                        File.WriteAllBytes(Path.Combine("ClientDownloads", "downloaded_image.png"), imageData);
                    }
                    else
                    {
                        ContentBox.Text += "Server: " + response + "\n";

                        // Save text file to client directory
                        Directory.CreateDirectory("ClientDownloads");
                        File.WriteAllText(Path.Combine("ClientDownloads", "downloaded_file.txt"), response);
                    }
                }
            }
            catch (Exception ex)
